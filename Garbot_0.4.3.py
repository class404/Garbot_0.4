...
def get_location():
    try:
        ...
        return {"error": f"Unable to fetch location data: {e}"}
...
def weather(city = ""):
    api_key = "73282eb088a69eb3419d21ba54751e7b"
    if city == "":
        city = get_location()[0]
...
while command != "exit":
    ...
    if command == "date":
        ...
        if time_counter == 1:
            ...
            speak(f"Its {time.strftime("%I:%M:%S %p")}")
        ...
        try:
            if answer == "y":
                ...
                temp = weather(city)[0]
                max_temp = weather(city)[1]
                min_temp = weather(city)[2]
            speak(f"so the tempreture is {temp :.2f} Kelvin or {temp - 273.15:.2f} in Celsius or {(temp - 273.15) * 9/5 + 32:.2f} in Fahrenheit")
            speak(f"with the max tempreture for today being {max_temp:.2f} in Kelvin or {max_temp - 273.15:.2f} in Celsius or {(max_temp - 273.15) * 9/5 + 32:.2f} in Fahrenheit")
            speak(f"while the minimum being {min_temp:.2f}in Kelvin or {min_temp - 273.15:.2f} in Celsius or {(min_temp - 273.15) * 9/5 +32:.2f} in Fahrenheit.")
